 ...
 * 使用 Seal 加密数据
 * @param data - 要加密的数据
 * @param password - 密码（可选，如果不提供则生成随机密钥）
 * @returns 加密结果 { encryptedBlob, key, iv }
 */
...
/**
 * 使用 Seal 解密数据
 * @param encryptedBlob - 加密的数据
 * @param keyBase64 - Base64 编码的密钥
 * @returns 解密后的数据
 */
export async function decryptWithSeal<T = unknown>(
  encryptedBlob: Blob,
  keyBase64: string
): Promise<T> {
  try {
...
export async function decryptWithPassword<T = unknown>(
  ...
  try {
    ...
    // 使用密钥解密
    return await decryptWithSeal<T>(encryptedBlob, keyBase64);
  } catch (error) {
    console.error('❌ Decryption with password failed:', error);
    const message = error instanceof Error ? error.message : 'Unknown error';
    throw new Error(`Password decryption failed: ${message}`);
  }
}
