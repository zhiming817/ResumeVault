...
import { fetchUserAllowlists } from '@/app/lib/utils/allowlistUtils';
...
interface EncryptionModeSelectorProps {
  ...
  setUseSealEncryption: (value: boolean) => void;
  encryptionMode: 'allowlist' | 'subscription';
  setEncryptionMode: (value: 'allowlist' | 'subscription') => void;
  allowlistId: string;
  setAllowlistId: (value: string) => void;
  capId: string;
  setCapId: (value: string) => void;
...
export default function EncryptionModeSelector({
  ...
  const [userAllowlists, setUserAllowlists] = useState<AllowlistInfo[]>([]);
  ...
  const handleAllowlistChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-gray-900 mb-1">📋 Allowlist Mode</div>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  />
                  <div className="flex-1">
                    <div className="font-semibold text-gray-900 mb-1">💰 Subscription Mode</div>
                    <div className="text-xs text-gray-600 space-y-1">
          ...
          {encryptionMode === 'allowlist' && (
            <div className="space-y-4">
              <div>
                  ...
                  >
                    ...
                    {userAllowlists.map((list) => (
                      <option key={list.allowlistId} value={list.allowlistId}>
                        {list.name} ({list.allowlistId.slice(0, 6)}...
                        {list.allowlistId.slice(-4)})
                      </option>
                    ))}
                  </select>
                ) : (
                ...
                <p className="text-xs text-blue-600 mt-1">Controls who can access your resume</p>
              </div>
              ...
              <div className="bg-white p-4 rounded border border-blue-200">
                ...
                <div className="mt-3">
                  <button
                    type="button"
                    onClick={() => router.push('/resume/allowlist')}
                    disabled={isSubmitting}
                ...
                <div className="text-xs text-orange-700 space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">
                      {(parseFloat(subscriptionPrice || '0') * 10).toFixed(9)} SUI
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>100 subscriptions:</span>
                    <span className="font-semibold">
                      {(parseFloat(subscriptionPrice || '0') * 100).toFixed(9)} SUI
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white p-4 rounded border border-blue-200">
            ...
            <div className="mt-3">
              <button
                type="button"
                onClick={() =>
                  window.open('https://docs.walrus.site/walrus-sites/seal.html', '_blank')
                }
                disabled={isSubmitting}
